...
def train_general(args):
  ...
  for epoch_i in range(args.num_epochs):
    ...
    while t * args.train_batch_size < args.datasize:
      ...
      for example in train_loader_labeled:
        if train_loader_new is not None:
          if 0 <= args.new_prob and args.new_prob <= 1 and len(train_loader_new) >= len(train_loader_labeled) / 300: # args.new_prob should be large, e.g., 0.9.   len(train_loader_new) >= len(train_loader_labeled) / 3 means the new data should be sufficient > 25 % of total
